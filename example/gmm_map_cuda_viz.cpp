...
public:
    ReconstructionWindow(const std::string& dataset_name,
            ...
            : gui::Window("GMMap Construction (CUDA)",
        ...
        fixed_props_->AddBool("Fuse GMM across frames",
                              ...
                              "Local or global GMM");
        fixed_props_->AddBool("Enable GPU acceleration", &prop_values_.enable_cuda, true,
                              "Use CUDA in mapping");
    ...
    void UpdateMain() {
        ...
        */
        ...
        auto gmm_map = std::make_shared<gmm::GMMMapCuda>(map_parameter, map_cuda_parameter,
                                                         &prop_values_.update_obs_gmms, &prop_values_.update_free_gmms,
                                                         &prop_values_.fuse_gmm_across_frames,
                                                         &prop_values_.enable_cuda);
        ...
        while (true) {
            if (!is_started_ || !is_running_ || is_done_) {
                ...
                gui::Application::GetInstance().PostToMainThread(
                        this, [this, traj, frustum, coord_axes, gmm_map_viz, T_eigen, env_bbox,
            ...
            if (!is_done_) {
                if (idx == 0){
                    std::this_thread::sleep_for(std::chrono::milliseconds(std::max<int>(500, prop_values_.sleep_ms)));
                } else {
                ...
                if (dataset_name_ == "tartanair") {
                    ...
                    if (dataset_name_ == "stata"){
                        ...
                        depthmap = Eigen::Map<MatrixXf, 0, Stride<Dynamic, Dynamic>>(depthmap_full.data(),
                                                                                        ...
                                                                                        Stride<Dynamic, Dynamic>(stride * depthmap_full.rows(), stride)) / dataset_param::scale;
                        *input_depth = input_depth->Resize(dataset_param::sampling_rate);
                    } else {
                        depthmap = dutil::TensorToEigenMatrixColMajor<float>(depth_tensor.Reshape({t_shape[0],t_shape[1]}))/dataset_param::scale;
                    }
                }
                ...
                gmm_map->insertFrameCuda(depthmap,curPose);
                ...
                // Transfer the point cloud ownership
                ...
                if ((prop_values_.update_surface &&
                    ...
                    frame_idx == fmin(depth_files.size() - 1, start_frame_idx + num_frames - 1)) {
                    std::lock_guard<std::mutex> locker(surface_.lock);
                    surface_.pcd =
                            model_->ExtractPointCloud(std::min<float>(idx, prop_values_.truncation_weight),
                                                      prop_values_.pointcloud_size)
                                    .To(core::Device("CPU:0"));
                    cur_env_max_bound = core::eigen_converter::TensorToEigenMatrixXf(surface_.pcd.GetMaxBound().Reshape({3,1}));
                    cur_env_min_bound = core::eigen_converter::TensorToEigenMatrixXf(surface_.pcd.GetMinBound().Reshape({3,1}));
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 7 || utility::ProgramOptionExistsAny(argc, argv, {"-h", "--help"})) {
        std::cout << "Input format: dataset dataset_info_path start_frame_idx num_frames --device [CPU:0 or CUDA:0]" <<std::endl;
        return 1;
    }
    ...
    app.AddWindow(std::make_shared<ReconstructionWindow>(dataset_name, start_frame_idx,
                                                         num_frames, device_code, mono));
    app.Run();
}
